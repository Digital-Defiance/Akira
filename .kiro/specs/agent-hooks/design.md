...
## Overview
Implement an agent hooks subsystem as a VS Code extension module that loads workspace-level hook declarations (.kiro/hooks.json), registers VS Code event listeners, and runs prompts in the background via a concurrent, cancelable, retryable execution engine. Design goals: safety (secrets redaction, git safeguards), low-latency registration/loading, non-blocking background execution, observability (output channel logs + structured execution records), and testability.

## System Architecture

Major components
- Config Loader
  - Loads and validates `.kiro/hooks.json` within workspace root.
...
Data flow
1. File read (.kiro/hooks.json) → validate → normalized Hook objects.
...
Internal
- File watcher for `.kiro/hooks.json`
...
- Structured logs as JSON lines optionally written to workspace `.kiro/logs/hooks.log`
...
```typescript
// typescript
type TriggerType = "onDidSaveTextDocument" | "onWillSaveTextDocument" | "gitCommit" | string;
...
Storage mechanism
- Primary storage: workspace file `.kiro/hooks.json`
- In-memory normalized map per workspace (non-persistent)
- Optional: append-only log file `.kiro/logs/hooks.log` for structured run history
...
- Avoid persisting raw prompt content containing secrets to logs; redact before writing to `.kiro/logs` if enabled.
- For git triggers require allowGit=true and repoRoot presence to avoid accidental runs during commits.

Input validation
- Strict JSON Schema validation for `.kiro/hooks.json`.
...
- Schema versioning for `.kiro/hooks.json` with "schemaVersion" field to allow migration paths.
