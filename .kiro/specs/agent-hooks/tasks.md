...
- [x] 1.3 Create shared types and schema files
  - Implementation notes: add `src/types.ts` exporting Hook, HookRuntime, ExecutionRecord, HookTriggerContext; add `schema/.akira.hooks.schema.json` copied from design.
  - Success criteria: types compile and schema file exists at `schema/.akira.hooks.schema.json`.
...
- [x] 2.1 Implement Config Loader (src/configLoader.ts)
  - Implementation notes: export `async loadHooks(workspaceRoot: Uri): Promise<Hook[]>`, event `onDidChange: Event<HookLoadResult>`. Use `workspace.fs.readFile`, parse JSON, validate via `ajv`, normalize defaults (concurrency, enabled, timeout). Add file watcher for `.akira/hooks.json`.
...
- [x] 2.2 Implement Secrets Redactor (src/secretsRedactor.ts)
...
- [x] 2.3 Implement Event Registry (src/eventRegistry.ts)
...
- [x] 2.4 Implement Hook Manager (src/hookManager.ts)
...
- [x] 2.5 Implement Prompt Runner skeleton (src/promptRunner.ts)
...
- [x] 3.1 Implement Hook Execution Engine (src/executionEngine.ts)
...
- [x] 3.2 Wire components in extension activation
  ...
  - Success criteria: on workspace open and sample `.akira/hooks.json` the manager registers listeners and a saved file event triggers enqueue flow (use log outputs to verify).
- [x] 3.3 Listener registration failure handling and hook disable logic
...
- [x] 3.4 Add redaction integration to logging and execution
  ...
  - Success criteria: logs captured in OutputChannel and `.akira/logs/hooks.log` (if enabled) never contain unredacted secrets matching configured patterns.

## Phase 4: Testing & Documentation

- [x] 4.1 Add unit tests for Config Loader, Event Registry, Hook Manager, Secrets Redactor
...
- [x] 4.2 Add unit/integration tests for Execution Engine and Prompt Runner
...
- [x] 4.3 Add integration test simulating VS Code events (vscode-test)
  - Implementation notes: create workspace fixture with `.akira/hooks.json`, run extension host tests to simulate `onDidSaveTextDocument` and `gitCommit` triggers. Use fake PromptRunner binary for deterministic results.
...
- [x] 4.4 Document usage and developer notes (README)
  - Implementation notes: add `docs/agent-hooks.md` describing `.akira/hooks.json` schema, activation flow, debug flags, and how to run tests. Include example hooks file and commands to enable structured log file `.akira/logs/hooks.log`.
  - Success criteria: README explains how to add hooks, debug, run tests and how secrets redaction works.
- [x] 4.5 Code review & release checklist
...
- [x] 4.6 (Optional) Structured log file writer to `.akira/logs/hooks.log`
...
- [x] 4.7 (Optional) Telemetry & metrics hooks
...
- [x] 4.8 (Optional) Debounce/coalesce high-frequency triggers
