...
- Checkpoint Manager: creates/restores markdown checkpoints under `.kiro/checkpoints/<session-id>/`.
- Session Manager: creates session files in `.kiro/sessions/<session-id>/session.md`, persists history, decisions, rollbacks, API logs.
...
- API Logger: logs LLM/external API calls to `.kiro/sessions/<session-id>/api-calls.md`.
...
- Internal:
  - write to `.kiro/sessions/<session-id>/api-calls.md`
...
- Files under workspace:
  - .kiro/sessions/<session-id>/{session.md,decisions.md,history.md,rollbacks.md,api-calls.md}
  - .kiro/checkpoints/<session-id>/phase-<N>.md
...
Data protection
- Session & checkpoint files stored in workspace `.kiro/` (user-visible); sensitive content flagged in logs and optionally redacted by configuration.
